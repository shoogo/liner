...
5. 제한사항
 ...
 > + 예외처리
 > + 테마 삭제
 > + Response data 양식을 정확하게 맞추지 못함. 객체가 아닌 Json 객체를 생성하여 객체를 Json 객체로 파싱하면 가능하나 시간 문제로 객체 형태로 넘겨, 불필요 데이터가 포함됨.
 > + ORM 기준 객체 생성을 하였으나 주고 받는 데이터가 설계 양식과 달라 불가피하게 ORM 형태를 유지하지 못함
